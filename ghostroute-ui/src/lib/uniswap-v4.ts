...
export const fetchPoolsFromEvents = async (
  ...
  try {
    ...
    const startBlock = fromBlock ?? (currentBlock > BigInt(50000) ? currentBlock - BigInt(50000) : BigInt(0));

    // Chunk size of 1000 blocks to stay under RPC limits (GetBlock has 1500 limit)
